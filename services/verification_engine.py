...
def run_bidder_verification(tender_id, bidder_id, is_reverification=False):
    ...
    for req in requirements:
        ...
        weight = float(req["scoring_weight"] or 10.0)
        ...
        applicability = evaluate_requirement_applicability(req, bidder, tender)
        eval_res["evidence"]["applicability"] = applicability
        if applicability["status"] in ("NOT_APPLICABLE", "EXEMPTED", "WAIVED"):
            eval_res["status"] = applicability["status"]
            eval_res["score_awarded"] = 0
            eval_res["explanation"] = applicability["reason"]
            continue
        if applicability["status"] == "UNKNOWN":
            eval_res["status"] = "NEEDS_REVIEW"
            eval_res["score_awarded"] = weight * 0.5
            eval_res["issues"].append(applicability["reason"])
            eval_res["explanation"] = applicability["reason"]
