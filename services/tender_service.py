...
def create_corrigendum(tender_id, officer_id, reason, updated_description=None, updated_turnover=None, updated_local_content=None):
    ...
    old_reqs = query_db(
        """SELECT * FROM requirements WHERE tender_id = ?
           AND tender_version_id = ?""",
        (tender_id, previous_version["id"] if previous_version else None)
    )
    for req in old_reqs:
        ...
        execute_db(
            ...
            INSERT INTO requirements (
                ...
                original_ai_output, review_status, scoring_weight, scoring_policy, requirement_fingerprint
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                ...
                req["structured_criteria"], req["source_clause"], req["source_page"], req["extraction_confidence"],
                req["extraction_source"], req["applicability_conditions"], req["exemption_conditions"],
                req["original_ai_output"], req["review_status"], req["scoring_weight"], req["scoring_policy"],
                req["requirement_fingerprint"]
            )
        )
