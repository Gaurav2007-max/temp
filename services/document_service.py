...
def _document_integrity(storage_path, bidder_id, tender_id):
    with open(storage_path, "rb") as document_file:
        file_bytes = document_file.read()
    sha256_hash = hashlib.sha256(file_bytes).hexdigest()
    duplicate = query_db(
        "SELECT id FROM documents WHERE bidder_id = ? AND tender_id = ? AND sha256_hash = ? ORDER BY id ASC LIMIT 1",
        (bidder_id, tender_id, sha256_hash),
        one=True
    )
    try:
        reader = pypdf.PdfReader(storage_path)
        metadata = {str(key): str(value) for key, value in (reader.metadata or {}).items()}
        raw_document = file_bytes[:]
    except Exception:
        metadata = {}
        raw_document = file_bytes

    signature_status = "DETECTED_UNVERIFIED" if b"/ByteRange" in raw_document or b"/Sig" in raw_document else "NOT_DETECTED"
    return {
...
def _validity_from_fields(fields):
    raw_date = next(
        (fields.get(key) for key in ("authorization_valid_till", "valid_until", "valid_till", "expiry_date") if fields.get(key)),
        None
    )
    if not raw_date:
        return "NO_EXPIRY_FOUND", None
    parsed = None
    for fmt in ("%Y-%m-%d", "%d-%m-%Y", "%d/%m/%Y", "%Y/%m/%d"):
        try:
            ...
            continue
    if not parsed:
        return "UNKNOWN", str(raw_date)
    if parsed < date.today():
        return "EXPIRED", parsed.isoformat()
    warning_days = int(os.environ.get("EXPIRY_WARNING_DAYS", "30"))
    if parsed <= date.today() + timedelta(days=warning_days):
        return "EXPIRING_SOON", parsed.isoformat()
...
def save_requirement_document(bidder_id, tender_id, requirement_id, file_obj, declared_doc_type=None, replace_document_id=None):
    ...
    integrity = _document_integrity(storage_path, bidder_id, tender_id)
    validity_status, valid_until = _validity_from_fields(fields)
    ...
    doc_id = execute_db(
        ...
        INSERT INTO documents (
            ...
            page_count, extraction_method, sha256_hash, duplicate_status, duplicate_of_id,
            validity_status, valid_until, tamper_indicators, file_metadata,
            digital_signature_status, qr_detected
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            ...
            pdf_res.get("page_count", 1), ext_method, integrity["sha256_hash"],
            integrity["duplicate_status"], integrity["duplicate_of_id"], validity_status,
            valid_until, json.dumps(integrity["tamper_indicators"]),
            json.dumps(integrity["file_metadata"]), integrity["digital_signature_status"],
            integrity["qr_detected"]
        )
    )
    ...
    return True, {
        ...
        "duplicate_status": integrity["duplicate_status"],
        "duplicate_of_id": integrity["duplicate_of_id"],
        "validity_status": validity_status,
        "valid_until": valid_until,
        "digital_signature_status": integrity["digital_signature_status"],
...
def get_bidder_document_checklist(tender_id, bidder_id):
    ...
    reqs = query_db(
        "SELECT * FROM requirements WHERE tender_id = ? AND COALESCE(review_status, 'PENDING') != 'REJECTED' ORDER BY is_mandatory DESC, id ASC",
        (tender_id,)
    )
